...
async def set_linked_steam(discord_id: str, steam_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE links SET status = ? WHERE discord_id = ? AND status = ?",
            ("relinked", discord_id, "linked")
        )
        await db.execute(
            "INSERT INTO links (code, discord_id, steam_id, status) VALUES (?, ?, ?, ?)",
            ("panel", discord_id, steam_id, "linked")
        )
        await db.commit()
...
async def user_profile(discord_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            """
            SELECT l.steam_id, l.status, p.eos_id
            FROM links l
            LEFT JOIN players p ON l.steam_id = p.steam_id
            WHERE l.id = (
                SELECT id
                FROM links
                WHERE discord_id = ?
                ORDER BY CASE WHEN status = 'linked' THEN 0 ELSE 1 END, id DESC
                LIMIT 1
            )
            """,
            (discord_id,)
